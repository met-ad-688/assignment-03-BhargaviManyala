...
## Interpretation
* The chart demonstrates that Business Intelligence Analysts and Data/Data Mining Analysts have the largest number of job postings but only moderate median salaries.
* Computer Systems Engineers/Architects earn the highest median salaries but with fewer postings.
* Overall, this indicates that high demand does not always align with the highest pay.
...
df = df.withColumn(
    "EDU_GROUP",
    when(col("EDUCATION_LEVELS_NAME").rlike("(?i)Associate|GED|No Education Listed|Highschool"), "Associate or Lower")
    .when(col("EDUCATION_LEVELS_NAME").rlike("(?i)Bachelor"), "Bachelor")
    .when(col("EDUCATION_LEVELS_NAME").rlike("(?i)Master"), "Master’s")
    .when(col("EDUCATION_LEVELS_NAME").rlike("(?i)PhD|Doctorate|professional degree"), "PhD")
    .otherwise(None)
)
...
np.random.seed(42)
df_pd["MAX_EXPERIENCE_JITTER"] = (
    df_pd["MAX_EXPERIENCE"] + np.random.uniform(-0.3, 0.3, len(df_pd))
)
...
fig = px.scatter(
    ...
    y="AVERAGE_SALARY",
    color="EDU_GROUP",
    hover_data=["OCCUPATION_NAME", "MAX_EXPERIENCE", "AVERAGE_SALARY"],
    title="Salary vs Experience by Education Level",
    color_discrete_map = {
    "Bachelor": "#1f77b4",        
    "Associate or Lower": "#d62728",  
    "Master’s": "#2ca02c",        
    "PhD": "#9467bd"              
}
)
...
fig.write_image("./output/Q4.png", width=900, height=600, scale=1)
...
## Interpretation
* Salaries tend to increase with years of experience, though variation exists across education levels.
* PhD and Master’s degree holders generally earn the highest salaries, while Associate or lower groups comparatively earn less.
...
df = df.withColumn(
    "REMOTE_GROUP",
    when(lower(trim(col("REMOTE_TYPE_NAME"))).rlike("hyb|mix|partial|split|combo"), "Hybrid")
    .when(lower(trim(col("REMOTE_TYPE_NAME"))).rlike("remote|wfh|home|tele"), "Remote")
    .otherwise("Onsite")
)
...
color_map = {"Onsite":"#2a9d8f", "Remote":"#e76f51", "Hybrid":"#264653"}  
...
fig = px.scatter(
    ...
    y="AVERAGE_SALARY",
    color="REMOTE_GROUP",
    category_orders={"REMOTE_GROUP": order},
    color_discrete_map=color_map,
    title="Salary vs Experience by Remote Work Type",
    labels={
...
fig.update_traces(marker=dict(size=7, opacity=0.85, line=dict(width=0)))

fig.write_html("./output/Q5.html")
fig.write_image("./output/Q5.png", width=1000, height=600, scale=1)
...
## Interpretation
* Professionals across all work types (Onsite, Remote, Hybrid) demonstrate similar salary trends with increasing experience.
* Hybrid roles appear less frequent but generally align with the salary range of Onsite and Remote roles.
